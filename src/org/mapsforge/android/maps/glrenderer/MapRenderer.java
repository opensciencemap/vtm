...
import org.mapsforge.android.maps.mapgenerator.TileCacheKey;
import org.mapsforge.android.maps.mapgenerator.TileDistanceSort;
import org.mapsforge.android.maps.utils.GlConfigChooser;
import org.mapsforge.android.maps.utils.GlUtils;
...
/**
 * TODO
 * - use proxy child/parent tile nearer to current tile (currently it is always parent first)
 * - use stencil instead of scissor mask for rotation
 * - draw up to two parents above current tile, maybe prefetch parent
 */
public class MapRenderer implements org.mapsforge.android.maps.MapRenderer {
	...
	public synchronized void redrawTiles(boolean clear) {
		...
		double x = MercatorProjection
				.longitudeToPixelX(mMapPosition.geoPoint.getLongitude(), zoomLevel);
		double y = MercatorProjection
				.latitudeToPixelY(mMapPosition.geoPoint.getLatitude(), zoomLevel);
	...
	private boolean drawPolygons(GLMapTile tile, int diff) {
		...
		if (useHalfFloat) {
			...
			GLES20.glVertexAttribPointer(gPolygonVertexPositionHandle, 2,
					GLES20.GL_FLOAT, false, 0, POLYGON_VERTICES_DATA_POS_OFFSET);
		}
	...
	private boolean drawLines(GLMapTile tile, int diff) {
		...
		if (useHalfFloat) {
			GLES20.glVertexAttribPointer(gLineVertexPositionHandle,
					2, OES_HALF_FLOAT, false, 8,
					LINE_VERTICES_DATA_POS_OFFSET);

			GLES20.glVertexAttribPointer(gLineTexturePositionHandle,
					2, OES_HALF_FLOAT, false, 8,
					LINE_VERTICES_DATA_TEX_OFFSET >> 1);
		} else {
			GLES20.glVertexAttribPointer(gLineVertexPositionHandle,
					2, GLES20.GL_FLOAT, false, 16,
					LINE_VERTICES_DATA_POS_OFFSET);
